...
3. 참고할 데이터 범위 : 먼저 최종목표와 가장 연관도가 높은 데이터를 Vector DB에서 찾고, 현재 날짜로부터 가장 가까운 작성일자를 가진 내용에 더욱 가중치를 두어 참조. 특정 보고서 또는 게시물을 지정하였다면 해당 보고서의 Chunk 내용만을 참고하고, 나머지는 사전 학습된 내용을 참고
4. 역할부여 : 매우 똑똑하고 전문적인 금융 전문가 애널리스트
5. 배경정보 : 배경지식이 없는 신규 고객에게 포트폴리오 내 주요 종목을 쉽게 설명하고 이해시키기 위함
6. 글의 종류 : 메모
7. 결과물의 형식 : Bullet Point From 3 Upto 5개, Sub-topic은 표시하지 말 것. 각 Bullet Point별 문장의 길이는 From 35 Upto 70글자 범위
...
12. 답변에 대한 해설 : 불필요
13. 출처표시 : 참고한 출처들은 결과물의 마지막 부분에 표시하고, '제목, 작성자, 작성일자'의 형태로 출력할 것""", language="plaintext")
...
6. 글의 종류 : 메모
7. 결과물의 형식 : Bullet Point From 3 Upto 6개, 각 Bullet Point별 문장의 길이는 From 20 Upto 60글자 범위, Sub-topic은 표시하지 말 것. 
...
14. 출처표시 : No In-context source, 결과물의 마지막 부분에만 표시하고, '제목, 작성자, 작성일자'의 형태로 출력할 것""", language="plaintext")

    with st.expander("3. VIS 보고서 내용 요약(서술식)"):
        st.code("""0. 사전질문 : 가장 최근에 작성된 '한양이엔지' 보고서는 뭐지? 

1. 최종목표 : 2024년 6월 20일자 '한양이엔지' 보고서 내용 요약 및 정리
2. 추가 데이터 제공 여부 : RAG 목적의 OpenAI Assistant API Vector DB
3. 참고할 데이터 범위 : 먼저 최종목표와 가장 연관도가 높은 데이터를 Vector DB에서 찾고, 현재 날짜로부터 가장 가까운 작성일자를 가진 내용에 더욱 가중치를 두어 참조. 특정 보고서 또는 게시물을 지정하였다면 해당 보고서의 Chunk 내용만을 참고하고 나머지 Vector DB 및 사전 학습된 내용은 참고 범위에서 배제
4. 역할부여 : DB를 전체적으로 빠짐없이 뒤져서 요청한 정보를 자연어 유사어와 전문용어까지 고려하여 찾아올 수 있는 검색엔진이자, 체계적이고 깔끔하게 정리를 잘 해내는 애널리스트
5. 글의 종류 : 메모
6. 결과물의 형식 : 최종목표에서 지정된 보고서의 내용을 서술식으로 요약정리. 수치적인 내용 보다는, 핵심 개념과 논리 위주로 작성. 내용이 복잡하고 길 경우에는 Sub-topic별로 구분.
7. 결과물의 분량 : From 900 Upto 1500 글자 범위 내
8. 필수 포함 내용 : 종목명, 작성자, 작성일, 목표주가, 투자의견 / 필수 포함 내용은 결과물의 가장 처음에 표시
9. 문체 : 개조식, 명사형종결어미
10. 작성언어 : 한국어
11. 제한사항 : Bullet Point 방식을 사용하지 않음. 가급적 수치는 내용에 포함하지 않도록 하고, 꼭 필요한 수치는 반드시 Vector DB 내에서만 직접적으로 인용, 숫자는 절대로 자의적으로 반올림/올림/버림 하지 말고 출처 내용에서 그대로 표시할 것
12. 출처표시 : No In-context source, 결과물의 마지막 부분에만 표시하고, '제목, 작성자, 작성일자'의 형태로 출력할 것""", language="plaintext")
...
4. 역할부여 : DB를 전체적으로 빠짐없이 뒤져서 요청한 정보를 자연어 유사어와 전문용어까지 고려하여 찾아올 수 있는 검색엔진이자, 체계적이고 깔끔하게 정리를 잘 해내는 애널리스트
5. 글의 종류 : 메모
6. 결과물의 형식 : 최종목표에 부합하는 내용을 서술형 방식으로 요약정리. 결과물의 분량이 1500글자 이내라면, Sub-topic으로 내용을 분류하지 말 것.
7. 결과물의 분량 : From 500 Upto 1000 글자 범위 내
8. 문체 : 개조식, 명사형종결어미
9. 작성언어 : 한국어
10. 제한사항 : 꼭 필요한 수치는 반드시 Vector DB 내에서만 직접적으로 인용, 숫자는 절대로 자의적으로 반올림/올림/버림 하지 말고 출처 내용에서 그대로 표시할 것
11. 출처표시 : 결과물의 마지막 부분에 표시하고, '제목, 작성자, 작성일자'의 형태로 출력할 것""", language="plaintext")
...
7. 결과물의 분량 : Bullet Point From 4 upto 8개 범위 내, 각 Bullet Point별 문장의 길이는 20~60글자 범위
8. 문체 : 개조식, 명사형종결어미
9. 작성언어 : 한국어
10. 제한사항 : 꼭 필요한 수치는 반드시 Vector DB 내에서만 직접적으로 인용, 숫자는 절대로 자의적으로 반올림/올림/버림 하지 말고 출처 내용에서 그대로 표시할 것
11. 출처표시 : 결과물의 마지막 부분에 표시하고, '제목, 작성자, 작성일자'의 형태로 출력할 것""", language="plaintext")
...
initial_message = (
    "저는 VIP AI(Combined Ver.) 입니다. VIS DB의 내용을 구석구석 뒤져서 최선을 다해 답변드리겠습니다. "
